package com.brain.das.web.rest;
...
import com.brain.das.config.KafkaProperties;
...
class DasKafkaResourceIT {
    ...
    void setup() {
        ...
        DasKafkaResource kafkaResource = new DasKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/das-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/das-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
